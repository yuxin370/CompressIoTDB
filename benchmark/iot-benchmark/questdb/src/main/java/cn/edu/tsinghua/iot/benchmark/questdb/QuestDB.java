...
package cn.edu.tsinghua.iot.benchmark.questdb;
...
public class QuestDB implements IDatabase {
      ...
      try (Statement statement = connection.createStatement()) {
        for (DeviceSchema deviceSchema : schemaList) {
          ...
          for (int index = 0; index < sensors.size(); index++) {
            String dataType = typeMap(sensors.get(index).getSensorType());
            create.append(sensors.get(index).getName());
            create.append(" ");
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      for (Record record : batch.getRecords()) {
        ...
        for (int i = 0; i < record.getRecordDataValue().size(); i++) {
          ...
          switch (typeMap(sensors.get(i).getSensorType())) {
            case "BOOLEAN":
  ...
  public Status preciseQuery(PreciseQuery preciseQuery) {
    ...
    String sqlHead = "SELECT " + sensors.get(0).getName();
    for (int i = 1; i < sensors.size(); i++) {
      sqlHead += ", " + sensors.get(i).getName();
    }
    String sql =
        sqlHead
            + " FROM "
  ...
  public Status expValueQuery(ExpValueQuery expValueQuery) {
    Exception e = new TsdbException("questdb don't support this kind of query");
