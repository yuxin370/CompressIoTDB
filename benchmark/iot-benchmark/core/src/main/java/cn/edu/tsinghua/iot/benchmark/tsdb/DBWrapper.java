...
public class DBWrapper implements IDatabase {
  ...
  public Status expValueQuery(ExpValueQuery expValueQuery) {
    Status status = null;
    Operation operation = Operation.EXP_VALUE_QUERY;
    String device = "No Device";
    if (expValueQuery.getDeviceSchema().size() > 0) {
      device = expValueQuery.getDeviceSchema().get(0).getDevice();
    }
    try {
      List<Status> statuses = new ArrayList<>();
      for (IDatabase database : databases) {
        long start = System.nanoTime();
        status = database.expValueQuery(expValueQuery);
        long end = System.nanoTime();
        status.setTimeCost(end - start);
        handleQueryOperation(status, operation, device);
        statuses.add(status);
      }
      doComparisonByRecord(expValueQuery, operation, statuses);
    } catch (Exception e) {
      handleUnexpectedQueryException(operation, e, device);
    }
    return status;
