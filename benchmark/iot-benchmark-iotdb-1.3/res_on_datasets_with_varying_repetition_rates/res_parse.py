...
def write_res_line(outfile,res_matrix,type_idx):
    ...
    outfile.write(","+str(round(((res_data[0]-res_data[2])/res_data[0]),4)))
    outfile.write(","+str(round(((res_data[1]-res_data[2])/res_data[1]),4)))
    outfile.write(","+str(round(((res_data[3]-res_data[2])/res_data[3]),4)))
    outfile.write("\n")

def write_res(outfile,tag,res_matrix):
    print(res_matrix)
    outfile.write(tag+",baseline,plain,ours,nolazy,ratio,ratio_p,ratio_nolazy\n")
...
def calculate_res(path,tag,outfile): 
    ...
    for i in range(len(content)):
        ...
        # elif (line.startswith("VALUE_RANGE")):
        #     line = line.strip('\n').strip('s').strip(" ")
        #     for i in range(40):
        #         line = line.replace("  "," ")
        #     VALUE_RANGE.append((float)(line.split(' ')[2]))
        #     print("VALUE_RANGE",(float)(line.split(' ')[2]))
...
def travers_calc():
    ...
    for filepath,dirnames,filenames in os.walk(r"./"):
        ...
        for filename in filenames:
            ...
            if(not filename.startswith("res_parse") and not filename.startswith("statics")):
                print(respath)
